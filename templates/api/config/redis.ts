...
export interface RedisConfig {
	...
	url?: string; // Support for REDIS_URL (used by Heroku)
}

// Parse REDIS_URL if available (Heroku sets this automatically)
const parseRedisUrl = (url: string) => {
	try {
		...
		logger.info(`Redis URL parsed: host=${config.host}, port=${config.port}`);
		return config;
	} catch (error) {
		logger.warn("Failed to parse REDIS_URL, using default config");
		return null;
	}
};

// Priority: REDIS_URL (Heroku) > Individual env vars > localhost
logger.info(`Checking REDIS_URL: ${process.env.REDIS_URL ? 'SET' : 'NOT SET'}`);
const redisUrlConfig = process.env.REDIS_URL ? parseRedisUrl(process.env.REDIS_URL) : null;

const baseConfig = redisUrlConfig || {
...
logger.info(`Redis config: host=${baseConfig.host}, port=${baseConfig.port}`);
